...
const MAX_CHECKS = 30;                // paced hourly ≈ give up after ~a week
const CHECK_GRACE_MS = 2 * 3600000;   // gamma flags `closed` hours after endDate
const CHECK_SPACING_MS = 3600000;     // don't burn attempts every 15-min run
  ...
  try {
    ...
    for (const o of opportunities || []) {
      ...
      records.unshift({
        ...
        taScore: o.ta ? o.ta.score : null, taAlignment: o.taAlignment || 'unknown', fng: o.fng ?? null,
        ts: Date.now(),
    ...
    const due = records.filter(r =>
      r.outcome === null
      && r.checks < MAX_CHECKS
      && Date.parse(r.endDate) < now - CHECK_GRACE_MS
      && now - (r.lastCheckAt || 0) > CHECK_SPACING_MS);
    if (due.length) {
      const batch = due.slice(0, MAX_RESOLUTION_SLUGS);
      // closed=true: these records are all past endDate, so their markets have
      // resolved and dropped off gamma's default (open-only) slug endpoint.
      const markets = await fetchGammaBySlug([...new Set(batch.map(r => r.slug))], { closed: true });
      for (const r of batch) {
        const m = markets.get(r.slug);
        if (m) {
          const closed = m.closed === true || m.closed === 'True' || m.closed === 'true';
          try {
            const yes = parseFloat(JSON.parse(m.outcomePrices)[0]);
            // Settle on the official closed flag, or on a price pinned at
            // 0/1 after the end date — gamma often flags `closed` hours late.
            if (Number.isFinite(yes) && (closed || yes >= 0.995 || yes <= 0.005)) {
