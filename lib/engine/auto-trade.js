...
const FAST_GAME_RE = /\bvs\.?\s|\bO\/U\b|\(BO\d\)|Game \d|Map \d|Spread:|Exact Score|Home Runs|\b(?:win|lose|advance)\b[^?]*\bon \d{4}-\d{2}-\d{2}/i;

// Trust boundary for upstream prices: anything divided into a position size
// must be a finite in-range probability, or shares/balances get poisoned.
const validPrice = p => Number.isFinite(p) && p > 0.005 && p < 0.995;
...
export async function runAutoTrade(portfolio, watchlist, consensusSnapshot) {
  ...
  try {
    ...
    const fetchResults = await Promise.all(profitable.map(async u => {
      try {
        const pos = await fetchPositions(u.proxyWallet, 200, 'CURRENT', 'DESC');
        return { ok: true, positions: pos.filter(p => p.currentValue > 1).map(p => ({ ...p, traderWallet: u.proxyWallet })) };
      } catch { return { ok: false, positions: [] }; }
    }));
    const fetchOk = fetchResults.filter(r => r.ok).length;
    const allPositions = fetchResults.map(r => r.positions);
    ...
    // A partial data-api outage (rate-limited position fetches) makes
    // consensusCounts sparse or empty, which exit detection would read as
    // "every trader left" and liquidate the whole book. Only trust the counts
    // when most trader fetches succeeded; otherwise skip exits and keep the
    // previous snapshot.
    const dataHealthy = fetchOk >= Math.ceil(profitable.length / 2)
      && Object.keys(consensusCounts).length > 0;

    if (dataHealthy) {
      const exiting = detectConsensusExits(portfolio, consensusSnapshot, consensusCounts);
      for (const pos of exiting.slice(0, 3)) {
    ...
    // Update snapshot for next run (keep the old one when data was degraded)
    const newSnapshot = dataHealthy ? consensusCounts : consensusSnapshot;
    ...
    if (portfolio.open.length < MAX_POSITIONS) {
      ...
      for (const c of sorted) {
        ...
        if (!validPrice(c.curPrice)) continue;
      ...
      if (wallets.length) {
        ...
        for (const wr of watchResults) {
          ...
          for (const buy of wr.recentBuys) {
            ...
            if (!validPrice(buy.price)) continue;
        ...
        for (const wr of watchResults) {
          ...
          for (const pos of wr.open.filter(p => p.currentValue > 500)) {
            ...
            if (!validPrice(pos.curPrice)) continue;
    ...
    return { portfolio, watchlist, consensusSnapshot: newSnapshot, placed, watchlistPlaced, consensusExits, fetchOk, tradersTotal: profitable.length };
  } catch (e) {
