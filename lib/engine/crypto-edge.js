...
function computeTa(prices, volumes) {
  ...
  const rsi = gain === 0 && loss === 0 ? 50 : loss === 0 ? 100 : Math.round(100 - 100 / (1 + gain / loss));
  ...
  // Rising volume confirms the prevailing trend: one extra vote in its direction
  if (volRising === true) score += smaFast > smaSlow ? 1 : -1;
...
export async function scanCryptoEdges() {
  ...
  for (const m of markets) {
    ...
    // Extreme sentiment adds one vote to the stance (greed>=75 bullish,
    // fear<=25 bearish) — applied at scan time so the coin-stats cache
    // isn't frozen with a stale sentiment reading.
    let ta = stats.ta || null;
    if (ta && fng && (fng.value <= 25 || fng.value >= 75)) {
      const s = ta.score + (fng.value >= 75 ? 1 : -1);
      ta = { ...ta, score: s, label: s >= 2 ? 'bullish' : s <= -2 ? 'bearish' : 'neutral' };
    }
    // 'unknown' (no TA data) is distinct from 'neutral' (indicators balanced)
    // so the byTA calibration comparison stays interpretable.
    const taAlignment = !ta ? 'unknown'
      : ta.label === 'neutral' ? 'neutral'
