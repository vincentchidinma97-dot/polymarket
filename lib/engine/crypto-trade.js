...
export async function runCryptoTrade(portfolio, opportunities = null) {
  ...
  try {
    ...
    for (const o of opportunities) {
      ...
      // TA veto (Strategy toggle, default on): skip when trend/momentum
      // strongly oppose the bet's direction (|score| >= 2 the other way).
      // Neutral TA lets the vol-model edge decide alone.
      if (portfolio.cryptoTaFilter !== false && o.taAlignment === 'against' && o.ta && Math.abs(o.ta.score) >= 2) continue;
