...
export async function runCryptoTrade(portfolio) {
  ...
  let cryptoPlaced = 0;
  try {
    const { opportunities } = await scanCryptoEdges();
    ...
    for (const o of opportunities) {
      ...
      if (!Number.isFinite(daysOut) || daysOut <= 0 || daysOut > MAX_DAYS_OUT) continue;

      // Underpriced YES -> buy Yes; overpriced YES -> buy No
      const buyYes = o.edgeRaw > 0;
      const entry = buyYes ? o.yesRaw : o.noRaw;
      if (entry <= 0.03 || entry >= 0.95) continue; // no payoff or junk pricing
