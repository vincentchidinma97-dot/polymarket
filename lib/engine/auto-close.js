...
export function runAutoClose(portfolio) {
  ...
  // User-tunable levels (Strategy page): positive fractions, null = off.
  const tp = portfolio.takeProfitPct === null ? null : (portfolio.takeProfitPct ?? TAKE_PROFIT_PCT);
  const sl = portfolio.stopLossPct === null ? null : (portfolio.stopLossPct ?? -STOP_LOSS_PCT);
  ...
  for (let i = portfolio.open.length - 1; i >= 0; i--) {
    ...
    // A price pinned at 0/1 means the market has effectively settled even if
    // gamma hasn't flagged `closed` yet — capture it and free the slot.
    const nearSettled = pos.currentPrice != null
      && (pos.currentPrice >= 0.995 || pos.currentPrice <= 0.005);

    let reason = null;
    if (pos.resolved || nearSettled) {
      reason = 'resolved';
    } else if (tp !== null && pnlPct >= tp) {
      reason = 'take-profit';
    } else if (sl !== null && pnlPct <= -sl && !holdToResolution) {
      reason = 'stop-loss';
    } else if (tp !== null && !holdToResolution && pos.highWaterMark > pos.entry) {
      // tp === null means the user chose "hold to resolution" — no trailing exits either
